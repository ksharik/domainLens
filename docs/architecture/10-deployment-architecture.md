...
| Persistence | Stores repository/snapshot/run state, Evidence Graph, Finding Graph/revisions, human decisions, Domain Knowledge Model, diagnostics, and producer versions. | Durable, backed up, access-controlled, and abstracted behind application ports. | Initial lightweight engine and later Azure-managed relational service. |
