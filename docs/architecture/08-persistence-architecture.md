...
**CURRENT — Scanner 0.1:** canonical Evidence Graph JSON is written to a caller-selected file; the CLI can reopen and validate that file. There is no application database, durable pipeline state, Finding Graph, Human Context store, human-review store, or Domain Knowledge Model.
...
| Human Context / Domain Assertions | Human-supplied contextual knowledge with its scope, eliciting question, supplied statement, provenance, applicable concepts/findings, and status where appropriate | Immutable, versioned revisions; corrected or superseded, never rewritten |
| Human review decisions | Challenges, accept/reject actions, rationale, review state, and applicable actor/time | Append-only review and audit history, separate from Human Context |
...
`Human Context` is the working conceptual name; `Domain Assertion` is an alternative, and the exact
name and physical schema remain an **OPEN DECISION**. A conceptual record may include a stable
context/assertion ID, analysis or repository scope, the question or clarification request that
elicited it, the supplied statement, timestamp, revision/supersession relationship, applicable
concepts or findings, and status/validation where appropriate. An actor, session, or principal
reference is present only according to the identity model eventually approved. This concept does
not establish an authentication or multi-tenancy requirement.
...
flowchart LR
    ...
    Run --> Finding
    Run --> Context["Human Context revisions"]
    Context --> Finding
    Finding --> Review["Human review decisions"]
...
Generated reports and Markdown are replaceable projections. They must not be the only persisted representation of evidence, Human Context, findings, review decisions, or domain knowledge, and they must not silently combine Proposed DDD Design with recovered/as-is knowledge.

## Immutability, revision, and provenance

Immutable/versioned artifacts include snapshot manifests and source hashes, canonical Evidence Graph documents, Human Context revisions, individual finding revisions, human review decisions, DKM versions, and the version stamps needed to reproduce them. Correction creates a new version with lineage rather than altering the historical artifact.
...
Human Context is a fourth, separately persisted concern: it is not deterministic Evidence Graph
content, a model-generated interpretation, a finding classification, or a review-state transition.
A finding may link to both Evidence IDs and Human Context revision IDs, preserving the provenance of
each input without introducing a new epistemic classification.
...
- a Human Context correction or supersession publishes a new revision with lineage to the prior
  revision rather than editing it in place;
- a finding revision references an existing sealed ContextPack, its deterministic Evidence IDs and
  applicable Human Context revision IDs, and producer versions;
...
Application contracts should express repositories, snapshots, run state, evidence documents, Human Context revisions, finding revisions, human review decisions, knowledge-model versions, semantic views, and audit history without exposing a particular Azure SDK or database type. Queries and presentation models must preserve the view discriminator and provenance categories even if a practical deployment separates:
...
- optimistic concurrency for Human Context revisions and human-review updates while retaining
  separate histories;
...
Retention policies must separately cover source snapshots, snippets, Evidence Graphs, Human Context revisions, ContextPacks, model inputs/outputs, findings, human review decisions, DKM versions, diagnostics, and audit records. Encryption, deletion, legal and ownership/access isolation, backup/restore, geographic residency, and tenant isolation if multi-tenancy is selected are unresolved V1 design mechanisms. Private repository support additionally requires approved credential and source-egress policy.
...
- Evidence, Human Context, Finding, ContextPack, and DKM schema migration/compatibility policy,
  including provenance-category and semantic-view preservation.
- Human Context versus Domain Assertion naming, physical schema, scope/link model,
  status/validation semantics, conditional identity reference, and revision/supersession rules.
