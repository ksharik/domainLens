...
| Legacy symbol enrichment | Bind a flattened set of manifest-verified repository C# sources against an explicit, exact tool-owned framework reference catalog without evaluating repository MSBuild or admitting repository binaries/build extensions. M0 proves only the net472 synthetic-compilation slice with declared `Partial` source resolution. M2 exposes that same verified context to built-in analyzers and centrally permits `Exact` WCF source observations only for one deterministically selected `net472`/`v4.7.2` project context; project-faithful configuration, non-WCF semantic projection, and broader profiles remain open. |
| Classic WCF analyzer | Recognize the bounded M2 WCF source, `.svc`, configuration, and direct programmatic constructs and contribute normalized contract, operation, endpoint, binding, behavior and hosting evidence. It strictly decodes supported text artifacts and bounds persisted repository-controlled values. It does not establish runtime configuration or business meaning. |
...
| Generic `.config` | **V1 Partial** | Current M2 for the allowlisted `system.serviceModel` subset; broader configuration semantics planned | Current: strict UTF-8/BOM-marked UTF-16 decoding, hardened XML parsing, and bounded WCF services, clients, endpoints, bindings, behaviors, hosting, extensions, relationships, namespace metadata diagnostics, and XDT-control detection. Planned: non-WCF dependency, persistence, security, and configuration-to-code semantics. | Classic WCF analyzer and Relationship / persistence / behavioral evidence | Invalid/unsupported encodings and incompatible XML declarations produce typed diagnostics and no evidence from that artifact. No config execution, type activation, secret use, remote lookup or unrestricted value egress. XDT controls suppress promotion of that WCF section, but transforms are not applied and effective values may still depend on external files, inheritance or machine-level configuration. Declarative quality is independent of a C# target-framework profile. |
| `web.config` | **V1 Partial** | Current M2 for the allowlisted WCF subset; broader V1 evidence planned | Current: strictly decoded captured `system.serviceModel` service, endpoint, binding, behavior, extension, and service-activation declarations when the section has no XDT control; bounded XDT/namespace diagnostics. Planned: broader authentication/authorization, connection/dependency, and configuration-driven behavior. | Classic WCF analyzer and Relationship / persistence / behavioral evidence | It is not proof of deployed IIS state. XDT is detected but never applied. Inheritance, locked sections, environment selection, custom providers/modules and machine-level settings may prevent exact effective resolution. Secrets and personal data require redaction policy before persistence or model egress. |
| `app.config` | **V1 Partial** | Current M2 for the allowlisted WCF subset; broader V1 evidence planned | Current: strictly decoded WCF client/service endpoints and supported binding/behavior/hosting declarations, plus bounded XDT/namespace diagnostics. Planned: broader connection/dependency and configuration-driven behavior. | Classic WCF analyzer and Relationship / persistence / behavioral evidence | Build-time renaming, transform application, external `configSource` files, environment substitution and machine-level configuration may make the effective runtime configuration unknown. Unsupported or declaration-incompatible encodings are not guessed. |
...
For every source row below, trusted framework identity and repository-source quality are separate.
`Exact` source evidence requires exactly one deterministic `net472`/`v4.7.2` project context;
`DL4001` and `Partial` (or weaker) apply to unsupported, unknown, multiple, or conditional profiles.
This restriction does not downgrade independent declarative `.svc` or configuration evidence.
Repository-controlled WCF text is matched in full within manifest bounds, then persisted in at most
1,024 UTF-16 code units using an explicit truncation marker, `originalLengthUtf16`, and
`sha256Utf16` over the exact big-endian UTF-16 code-unit sequence. `DL4504` makes any abbreviation
visible and produces `PartialSuccess`.

| Artifact or concept | V1 treatment | Delivery state | Evidence contribution | Expected deterministic owner | Major limitations and security constraints |
|---|---|---|---|---|---|
| `.svc` | **V1 Partial** | Current M2 bounded rule | Static `ServiceHost` directive metadata (`Service`, `Factory`, `CodeBehind`, and `Language`), hosting node, and conservative link to a source implementation. | Classic WCF analyzer | Strict UTF-8 with/without BOM and BOM-marked UTF-16 LE/BE only; invalid or unsupported bytes produce typed diagnostics and no evidence from that file. No ASP.NET compilation or factory activation. Dynamic expressions, conflicting directives, build substitution, custom factory behavior and types not present in source remain unsupported or unresolved. |
...
- Invalid/unsupported text encodings and incompatible XML declarations produce typed diagnostics
  and no semantic evidence from the affected artifact; oversized persisted WCF text uses the
  explicit bounded marker/length/digest representation and produces `DL4504` rather than silently
  appearing complete.
...
3. **Safe symbol enrichment:** M0 proves one flattened repository-manifest C# compilation plus an
   ...
   the graph. Its centralized policy permits `Exact` source observations only for one
   deterministically selected `net472`/`v4.7.2` project, emits `DL4001` otherwise, and leaves
   independent declarative quality unchanged. Project-faithful compilation, non-WCF semantic
   projection, additional framework profiles, conditional compilation, broader coverage
   accounting, and unavailable repository/third-party types remain unresolved.
...
4. **Effective WCF/configuration resolution:** M2 implements the approved bounded source, `.svc`,
   ...
   Its strict UTF-only encoding contract rejects invalid/unsupported bytes and incompatible XML
   declarations without evidence promotion, while its 1,024-code-unit persistence representation
   deliberately abbreviates oversized values after full-value matching.
