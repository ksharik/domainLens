...
- human-provided domain context is provenance-bearing input, not an Evidence Graph observation or
  a review-state transition;
...
- coverage measurements and model self-assessment are not evidence, and product Confidence is not
  populated or exposed unless its calibration and use have passed the approved readiness gate; and
...
relationships. It is independent of semantic support, any available calibrated Confidence,
coverage, and completeness. Support and Confidence remain different concepts: Support describes
how well the cited material supports a claim, while Confidence would describe calibrated
reliability for a finding type. Confidence is unavailable, not calibrated, or omitted from
user-facing presentation until a versioned calibration method, an applicable expert-reviewed
...
Repository evidence is not the only possible input to semantic reasoning. Where a domain expert or
other human supplies business knowledge that the repository cannot establish, DomainLens preserves
it as a durable provenance record, referred to here conceptually as **Human Context** or a
**Domain Assertion**. The exact name and physical schema remain open decisions.

Such a record may include a stable context/assertion ID, repository or analysis scope, the question
that elicited it, the supplied statement, an actor/session/principal reference under whatever
identity model is eventually approved, a timestamp, revision or supersession links, applicable
concepts or findings, and status or validation metadata where appropriate. This does not introduce
an authentication, identity, or tenancy requirement.

Human Context is versioned and auditable, may be corrected or superseded without rewriting history,
and may be included as a separately typed input in a sealed ContextPack. A semantic finding may
reference both deterministic Evidence IDs and Human Context IDs, while keeping those provenance
sources visibly distinct. Human Context is not inserted into the Evidence Graph, is not classified
as `Observed`, does not create a new epistemic classification, is not a model interpretation, and
is not the same record as acceptance, rejection, challenge, or any other review-state transition.
Repository prose also remains untrusted repository evidence; it must not masquerade as trusted
Human Context.
...
|---|---|---|---|---|
| Business Capability | Operations grouped by behavior and outcomes; participating rules, workflows, data, messages, external dependencies, and vocabulary; entry-point-to-effect traces | WCF operation evidence (M2); call, behavior, persistence, message, and side-effect extraction (M3 documented home; detailed extraction and acceptance contracts remain open); lexical/context retrieval | Domain discovery → Recovered (`Inferred`); alternatives may be reviewed by a human | **V1 — evidence gap.** Structural and WCF signatures alone cannot establish business capability. |
| Actor | Entry points; authentication/authorization context; callers and generated clients where available; operation usage; scheduled or system triggers; repository documentation only as untrusted evidence | WCF/service/config analyzer (M2); caller, security, and scheduled-trigger extraction (M3 documented home; detailed extraction and acceptance contracts remain open) | Domain discovery → Recovered (`Inferred`), often with Human Context | **V1 — human context likely; evidence gap.** External human roles are frequently absent from server source. |
| Use Case | Actor or trigger; initiating operation/message; ordered calls; rules; data reads/writes; state changes; result/fault; external effects | WCF operation analyzer (M2); call/behavior/data-flow/message/state/exception extraction (M3 documented home; detailed extraction and acceptance contracts remain open) | Domain discovery → Recovered (`Inferred`) | **V1 — evidence gap.** An operation name is not a complete use case. |
...
| Core / Supporting / Generic classification | Domain/subdomain evidence plus differentiation, strategic importance, replaceability/commodity signals, and organizational/business context | Repository evidence can establish implementation and dependency signals; versioned Human Context may supply business strategy where absent | Domain discovery → Recovered (`Inferred`) with explicit assumptions, alternatives, and separate Human Context references | **V1 — human context likely.** Source code usually cannot establish strategic importance by itself. |
| Bounded Context | Capability and vocabulary cohesion/conflicts; model meanings; rules; ownership; transactions; API/message contracts; security policy; dependencies; integration patterns; shared data | Composite graph over structural/WCF (M1/M2) and behavior, persistence, message, security, and coupling evidence (M3 documented home; detailed extraction and acceptance contracts remain open) | DDD interpretation may recover an enforced boundary (`Inferred`); DDD modeling may recommend one (`Proposed`) | **V1 — evidence gap.** Recovery and proposal must be separate findings; package boundaries are weak evidence only. |
...
| Repository | Query/write operations; aggregate/entity type usage; mapping/store access; unit-of-work or transaction participation; callers | Structural interfaces (M1); body, persistence, and transaction extraction (M3 documented home; detailed extraction and acceptance contracts remain open) | DDD interpretation → Recovered (`Inferred`) or DDD modeling → Proposed (`Proposed`) | **V1 — partial path plus evidence gap.** A `Repository` suffix is not sufficient; support depends on persistence artifacts. |
...
| Domain Event | Past-tense domain occurrence; publication; subscribers; payload; causal mutation; transaction timing; retry/delivery behavior | Message/publication/subscription, call, mutation, and transaction extraction (M3 documented home; detailed extraction and acceptance contracts remain open) | DDD interpretation → recovered event (`Inferred`) or DDD modeling → Proposed (`Proposed`) | **V1 — evidence gap; open artifact dependency.** A .NET event or `*Event` class is not automatically a Domain Event. |
| Handler | Registration/dispatch; consumed command/event/message; invoked behavior; side effects; retry/error semantics; transaction scope | Framework registration/config plus dispatch/call/message/body analysis (M3 documented home; registration/dispatch pattern contracts and acceptance criteria remain open) | DDD interpretation → Recovered (`Inferred`) or DDD modeling → Proposed (`Proposed`) | **V1 — evidence gap.** Milestone 3 provides the behavioral, message, side-effect, and transaction home, but does not yet define supported handler patterns or their extraction and acceptance contracts. |

## Business behavior

| Knowledge output | Minimum evidence needed | Deterministic source/analyzer or stage | Semantic reasoning stage | Horizon and sufficiency |
|---|---|---|---|---|
| Business Rule | Conditions, comparisons, constants, calculations, branches, decisions, exceptions/faults, affected state/data, and observable outcomes | Method-body control/data-flow, constant/configuration, exception, and call extraction (M3 documented home; detailed extraction and acceptance contracts remain open) | Domain discovery → Recovered (`Inferred`) | **V1 — evidence gap.** Milestone 3 now names conditions, comparisons, validation, and exceptions, but supported constructs, analysis depth, and acceptance criteria remain unspecified. |
| Invariant | Rule enforced across mutation paths; guarded state; invalid-state handling; creation/update/delete coverage; transaction/consistency scope; counterexamples | Mutation/control-flow, validation/exception, persistence, and transaction evidence (M3 documented home; detailed extraction and acceptance contracts remain open) | Domain discovery → Recovered (`Inferred`); DDD modeling may relate it to a proposed aggregate (`Proposed`) | **V1 — evidence gap.** One guard on one path cannot prove universal enforcement. |
...
| Workflow / Business Process | Trigger; ordered and branching call chains; participants; state transitions; reads/writes; messages; external calls; compensations and failure paths | WCF entry points (M2); interprocedural call/control-flow, state, message, side-effect, and exception extraction (M3 documented home; detailed extraction and acceptance contracts remain open) | Domain discovery → Recovered (`Inferred`) | **V1 — evidence gap.** A static dependency graph alone does not establish ordering or business process. |
| State Transition / Lifecycle | State representation; before/after assignments; guards; triggering operation/message; invalid transitions; persistence and transaction context | State-write/data-flow/control-flow plus persistence and call extraction (M3 documented home; detailed extraction and acceptance contracts remain open) | Domain discovery → Recovered (`Inferred`) | **V1 — evidence gap.** Enum declarations do not establish a state machine. |
| Decision | Branch inputs, condition, alternatives, selected outcomes, rule source, and affected behavior/data | Control/data-flow and constant/configuration extraction (M3 documented home; detailed branch/data-flow contracts and acceptance criteria remain open) | Domain discovery → Recovered (`Inferred`) | **V1 — evidence gap.** Milestone 3 provides a broad behavioral and condition-analysis home, but does not yet define the decision artifact, extraction, resolution, coverage, or acceptance contract. |
| Preconditions / Postconditions | Entry guards; input and prior-state checks; success/failure exits; resulting state, outputs, messages, and faults | Control/data-flow, validation, state, side-effect, and exception extraction (M3 documented home; detailed extraction and acceptance contracts remain open) | Domain discovery → Recovered (`Inferred`) | **V1 — evidence gap.** Signature types alone do not establish behavioral contracts. |
| Validation Rule | Input/state checks; validation APIs/attributes; branches; error codes/messages/faults; affected operation and bypass paths | Attribute/config plus method-body control/data-flow and exception extraction (M3 documented home; detailed extraction and acceptance contracts remain open) | Domain discovery → Recovered (`Inferred`) | **V1 — evidence gap.** Declarative attributes provide only partial coverage without imperative validation paths. |
...
| Transaction Boundary | `TransactionScope`, connection/ORM transactions, unit-of-work patterns, ambient transactions, commit/rollback, operation attributes/config, calls and messages inside scope | Transaction, call/control-flow, WCF, and persistence extraction (M3 documented home; detailed extraction and acceptance contracts remain open) | Domain discovery → Recovered (`Inferred`) | **V1 — evidence gap.** Persistence access alone does not establish a transaction boundary. |
...
1. **Traceability:** every source-backed assertion cites valid Evidence IDs present in the sealed
   ContextPack, with source spans/hashes and analyzer/rule versions preserved. A finding that also
   uses human-provided business knowledge cites separate Human Context IDs from that same sealed
   pack; those references never satisfy a deterministic-evidence requirement.
...
8. **Human context provenance:** strategic importance, external actors, organization ownership,
   policies, and other business knowledge supplied by a human remain identifiable through durable,
   versioned Human Context IDs. They are separate from Evidence Graph records, model interpretation,
   and review-state transitions, and correction creates a new revision or supersession relationship
   rather than rewriting history.
9. **Conditional Confidence:** Support, Coverage, Completeness, counterevidence, limitations, and
   Resolution Quality remain available where applicable. Confidence remains conceptually distinct
   but is unavailable, not calibrated, or omitted until the calibration and product-approval gate
   described above has passed; raw model self-confidence cannot populate it.
...
Separately, Milestones 6 and 7 cannot claim semantic-quality readiness against defined accuracy
targets—and cannot expose calibrated Confidence—until an applicable expert-reviewed corpus of
fixtures or version-pinned representative repositories, structured expected results, evaluation rubric, a versioned calibration
method and evaluated calibration results where Confidence is used, explicit product approval to
expose Confidence, and approved acceptance/regression thresholds where required all exist. This is
an acceptance prerequisite for the existing semantic-reasoning milestones, not a new milestone,
and this document does not invent numeric thresholds.

## Knowledge outputs with incomplete evidence contracts

Milestone 3 now supplies the documented V1 home for behavioral analysis, conditions, comparisons,
validation, exceptions, decisions, workflow and state, messages and side effects, scheduling,
security checks, dependencies, and coupling. The following output groups nevertheless lack
sufficiently detailed artifact, extraction, resolution, coverage, or acceptance contracts:
...
The most detailed currently identifiable V1 paths are structural declarations and dependencies
already supplied by Milestone 1 and WCF APIs/operations and contracts through Milestone 2. The
[V1 Analysis Coverage](01-v1-analysis-coverage.md) matrix records intended treatments for
persistence, configuration, generated, scheduled, and external artifacts. Milestone 3 now gives
the behavioral, lifecycle, service/message, security, ownership/consistency, and cross-cutting
groups above a broad evidence-capability home, including persistence and shared data. Strategic
structure and vocabulary additionally depend on composite and lexical/context-retrieval contracts.
Detailed read/write, transaction, behavioral, security, message, scheduling, coupling, resolution,
and lexical acceptance contracts remain underspecified and must be approved before the
corresponding evidence supply can be considered implementation ready.
...
- The vocabulary term/meaning/conflict representation and the role of provenance-bearing Human
  Context in definitions and strategic subdomain classification.
- The canonical name and schema for Human Context / Domain Assertions, including revision,
  supersession, status/validation, scope, optional identity references under a future approved
  identity model, and links to ContextPacks and findings.
- Per-output minimum evidence recipes, completeness rules, Support rules, the optional Confidence
  calibration and exposure decision, validation gates, and escalation criteria.
